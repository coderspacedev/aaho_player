...
class _AdvancedVideoPlayerState extends State<AdvancedVideoPlayer> {
  ...
  Future<void> _initializePlayer() async {
    ...
    await _player.setVolume(_volume * 100); // media_kit uses 0–100 range
    await _player.play();
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (_showControls && orientation == Orientation.portrait) _buildCenterControls(),
          if (_showControls && _isDragging)
            Positioned(left: _getOverlayLeft(), top: math.max(_dragStartPosition.dy - 40, 0), child: _buildDragOverlay()),

          if (_showControls) _buildBottomControls(),
          if (_showControls) _buildTopBar(context, orientation),
        ],
      ),
    );
  }
  ...
  Widget _buildTopBar(BuildContext context, Orientation orientation) {
    return Container(
      ...
      padding: EdgeInsets.symmetric(horizontal: orientation == Orientation.portrait ? 16 : 8),
      child: SafeArea(
        child: Row(
            ...
            builder: (context, snapshot) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Icon(playing ? Icons.pause : Icons.play_arrow, color: Colors.white, size: 60),
                ),
              );
            },
          ),
  ...
  Widget _buildDragOverlay() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Icon(_isBrightnessDrag ? Icons.brightness_medium : Icons.volume_up, color: Colors.white, size: 24),
          Text('${((_isBrightnessDrag ? _brightness : _volume) * 100).round()}%', style: const TextStyle(color: Colors.white)),
        ],
      ),
    );
  }
